...
from ai_engine import generate_market_news
...
def background_price_generator():
    ...
    # Let static files load instantly, but allow background routes to trigger our engine
    if request.path.startswith('/static'):
        return

    now = datetime.utcnow()

    # Running AI generator tick every 30 seconds
    if (now - last_update_time).total_seconds() >= 30:
        all_stocks = Stock.query.filter_by(ActiveStatus=True).all()
        if not all_stocks:
            return

        # Collect active symbols, send to Gemini, and parse the data wrapper
        available_tickers = [s.Ticker for s in all_stocks]
        ai_news = generate_market_news(available_tickers)

        impacted_stock = Stock.query.filter_by(Ticker=ai_news.ticker).first()

        if impacted_stock:
            # Commit the news headline to local table structure
            new_article = SimulatedNews(
                StockID=impacted_stock.StockID,
                Headline=ai_news.headline,
                Sentiment=ai_news.sentiment,
                Intensity=ai_news.intensity
             )
            db.session.add(new_article)

            # Formulate the pricing adjustments based on sentiment metrics
            if ai_news.sentiment == "POSITIVE":
                sentiment_direction = 1.0
            elif ai_news.sentiment == "NEGATIVE":
                sentiment_direction = -1.0
            else:
                sentiment_direction = 0.0

            if sentiment_direction != 0.0:
                change_pct = sentiment_direction * (ai_news.intensity * random.uniform(0.005, 0.01))
            else:
                change_pct = random.uniform(-0.002, 0.002)

            impacted_stock.CurrentPrice = round(impacted_stock.CurrentPrice * (1 + change_pct), 2)
            if impacted_stock.CurrentPrice < 1:
                impacted_stock.CurrentPrice = 1.00

            db.session.add(PriceHistory(StockID=impacted_stock.StockID, Price=impacted_stock.CurrentPrice))

        # Apply basic standard marketplace background drift to passive stocks
        for s in all_stocks:
            if impacted_stock and s.StockID == impacted_stock.StockID:
                continue

            drift_pct = random.uniform(-0.005, 0.005)
            s.CurrentPrice = round(s.CurrentPrice * (1 + drift_pct), 2)
            if s.CurrentPrice < 1:
                s.CurrentPrice = 10.00

            db.session.add(PriceHistory(StockID=s.StockID, Price=s.CurrentPrice))
...
def dashboard():
    ...
    live_news = SimulatedNews.query.order_by(SimulatedNews.Timestamp.desc()).limit(10).all()
...
def market_news():
    ...
    live_news = SimulatedNews.query.order_by(SimulatedNews.Timestamp.desc()).limit(25).all()
    return render_template('market_news.html', news_feed=live_news)


@app.route('/api/market_news')
def api_market_news():
    news_feed = SimulatedNews.query.order_by(SimulatedNews.Timestamp.desc()).all()
    news_list = []
    for article in news_feed:
        news_list.append({
            'Timestamp': article.Timestamp.strftime('%I:%M:%S %p'),
            'Ticker': article.stock.Ticker if article.stock else 'N/A',
...
def random_price_generator():
    # 🤖 Force our AI sentiment engine tick to evaluate right before returning prices
    background_price_generator()
    
    # Fetch the newly altered values from the database
    all_stocks = Stock.query.filter_by(ActiveStatus=True).all()
